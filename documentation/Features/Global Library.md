...
- **Cross-device translation cache (Supabase):** Translations sync to/from dedicated Supabase language tables (`translations_hindi`, `translations_telugu`, etc.) with language availability tracked in `book_languages` so re-opening the same document on another device skips redundant translation.
...
- **`src/lib/supabase.ts`** — server functions `fetchSupabaseLanguageBook`, `fetchSupabaseLanguagePage`, `saveSupabaseLanguagePage`, `batchSaveSupabaseLanguagePages`, and `fetchAvailableLanguagesForBook` reading/writing dedicated translation tables and `book_languages`.
