...
- While a user listens to a translated/explained page via TTS, the app translates the *next* page ahead of time so playback can continue without a pause.
...
1. [[RightPanel]] dispatches `doclens:ensure-page-ready` for the active page (and, once playback is under way with Continuous Play on, for the *next* page too) — see `src/components/RightPanel.tsx`.
